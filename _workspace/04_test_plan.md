...
```
            ...
            │  Auth · Pay · Refund │
            ├──────────────────┤
            │ Integration 20%  │  vitest + mocked OpenAI/Stripe/Supabase
...
| NFR-8 | Webhook signature 검증 | `stripe.webhooks.constructEvent` | unit: signature pass/fail | `tests/integration/stripe-webhook.test.ts` | 🟢 자동 |
...
| # | 시나리오 | 입력 | 기대 결과 | 유형 |
|---|---------|------|----------|------|
| P1 | Checkout single | plan=credit_single | 200 `{checkout_url}` → Stripe redirect | 수동 §5.3 #7 |
| P2 | Checkout weekly_pass | plan=weekly_pass | 동일 | 수동 |
| P3 | Checkout monthly_sub (mode=subscription) | plan=monthly_sub | 동일, subscription_data.metadata 포함 | 수동 |
| P4 | Webhook bad signature | invalid sig | 400 `ERR_WEBHOOK_SIGNATURE` | integration `stripe-webhook.test.ts` |
| P5 | Webhook valid signature | signed body | 200 `{received:true}` | integration |
| P6 | Webhook duplicate event | 동일 event.id | 200 `{received:true, duplicate:true}` (no second insert) | 수동 (멱등성) |
| P7 | checkout.session.completed + credit_single | event | `credit_ledger` `purchase_single +1` row | integration |
| P8 | checkout.session.completed + weekly_pass | event | `subscriptions` row, soft_cap=100, period_end=now+7d | integration |
| P9 | checkout.session.completed + monthly_sub | event | `subscriptions` row, soft_cap=500, period_end=now+30d | integration |
| P10 | invoice.paid (monthly renewal) | event | period_end +30d, usage_count=0, status='active' | integration |
| P11 | customer.subscription.deleted | event | status='canceled' | integration |
| P12 | charge.refunded (credit_single, ≤7d) | event | `credit_ledger` `refund_reversal -1` row, note "within 7d" | integration |
| P13 | charge.refunded (credit_single, >7d) | 8일 후 | 동일 ledger row, note "operator override after 7d" | integration |
| P14 | charge.refunded (pass/sub) | event | subscriptions.status='refunded' | integration |
| P15 | Unhandled event type | `payment_intent.created` | 200 silent no-op | integration |
...
| `src/lib/stripe/**` | 70% lines | 동일 | 4 이벤트 분기 |
...
- [ ] Buy Single → Stripe Checkout (test card `4242 4242 4242 4242`) → `/account?checkout=success` redirect → 크레딧 +1
- [ ] Account page: 활성 plan, 잔여 사용량, 만료일 정확히 표시
- [ ] Refund (Stripe Dashboard에서 클릭) → 30초 후 ledger에 `refund_reversal` row, account UI에 반영
- [ ] Logout → 보호 라우트 접근 시 `/login` redirect

### 7.2 보안 수동 점검 (`_workspace/06_review_report.md`와 cross-ref)

- [ ] DevTools → Sources → JS bundle에 `sk_live_`, `whsec_`, `service_role` 키워드 검색 → 0건
- [ ] 두 번째 Google 계정으로 첫 계정의 `/analyses/<id>` URL 직접 입력 → 404
- [ ] 음수 webhook signature 테스트: `curl -X POST .../api/webhooks/stripe -d '{}'` → 400
...
| `tests/integration/stripe-webhook.test.ts` | 10 | integration (sig + 4 events + idempotency + edge) |
...
| Stripe live 환불 테스트 | 수동 §7 | Stripe CLI `stripe trigger charge.refunded` CI step 추가 |
