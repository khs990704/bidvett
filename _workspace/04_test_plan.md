...
| # | 시나리오 | 입력 | 기대 결과 | 유형 | 위치 |
|---|---------|------|----------|------|------|
| A1 | Google OAuth 신규 가입 | `/login` → 동의 → callback | 302 → `/onboarding`, `credit_ledger` `free_grant +5` | 수동 | smoke §5.3 #2-3 |
...
| # | 시나리오 | 입력 | 기대 결과 | 유형 |
|---|---------|------|----------|------|
| AN1 | Safe job (T1 fixture) | upwork-sample.txt | 200 SHOW_REPORT, match_score 70~95, credit_after=4 | 수동 §5.3 #5 |
...
| AN5 | Pre-check: balance=0, no sub | poor user | 402 `ERR_OUT_OF_CREDITS` | 수동 (smoke 후 +5 소진) |
...
| P8 | `payment.succeeded` + weekly_pass | event | no-op; `subscription.active`가 구독 source of truth | integration |
| P9 | `subscription.active` + weekly_pass/monthly_sub | event | `subscriptions` row, soft_cap=100/500, period_end=next_billing_date, status='active' | integration |
| P10 | `subscription.renewed` (weekly/monthly renewal — 구 `invoice.paid` 흡수) | event | period_end=next_billing_date, usage_count=0, status='active', cancelled_at=null | integration |
| P11 | `subscription.cancelled` | event | cancelled_at 기록, status='active' 유지, period_end 보존 | integration |
| P12 | `refund.succeeded` (future refund sync) | event | `credit_ledger` `refund_reversal -1` row | integration |
| P13 | `refund.succeeded` (duplicate delivery) | same event id | idempotent no-op | integration |
...
- [ ] Dashboard: 잔여 크레딧 5 표시 (CreditBadge)
...
- [ ] Pricing 페이지: 3 plan card + Privacy/Terms footer links
