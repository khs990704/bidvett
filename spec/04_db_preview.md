...
부가적으로 `stripe_events` (멱등성 보장용 이벤트 로그) [가정].
...
erDiagram
  ...
  SUBSCRIPTIONS ||--o{ STRIPE_EVENTS : "originates from"
  CREDIT_LEDGER ||--o{ STRIPE_EVENTS : "originates from"
...
| `stripe_event_id` | text | nullable | 멱등성용 |
...
| `stripe_subscription_id` | text | nullable | monthly_sub만 |
| `stripe_checkout_session_id` | text | nullable | weekly_pass (one-time) 추적 |
| `created_at` | timestamptz | not null, default now() | |
...
| 컬럼 | 타입 | 제약 | 설명 |
|------|------|------|------|
| `id` | text | PK | Stripe event id (예: `evt_...`) |
| `type` | text | not null | `checkout.session.completed` 등 |
...
| `credit_ledger` | unique | `(stripe_event_id)` where stripe_event_id is not null | Webhook 멱등성 |
...
| `stripe_events` | btree | `(received_at desc)` | 운영 모니터링 |
