...
> Reference: `idea.md §6` 1~2주 차 랜딩 페이지 + Stripe + Supabase 첫날 연동, 3~4주 차 Reddit 트래픽, 5주차+ 구독 확장 → MVP 일정과 정렬.
...
|-------|------|----------|---------|
| **MVP (v0.1)** | 랜딩 + 인증 + 핵심 분석 + Stripe 결제 + 무료 3회 모델이 라이브 | FR-1 ~ FR-12 (P0 전부) | **2 weeks** (idea.md §6 1~2주 차) |
...
| 12 | `/api/checkout` + Stripe Webhook (`checkout.session.completed`, `charge.refunded`) + `stripe_events` 멱등성 | backend |
| 13 | E2E 결제 시나리오 (Stripe test mode) + Out-of-credits banner + Pricing 카드 연결 | qa + frontend |
| 14 | Production 도메인 + Stripe live keys + Supabase backup 설정 + 런칭 | devops |

### Weeks 3-4 — Growth & Polish (v1.0)

| Week | 작업 |
|------|------|
| 3 | Analysis History 리스트 + `/analyses/[id]` + Account Settings + Stripe Customer Portal 링크 |
| 3 | Email Notifications (Resend or Supabase SMTP) — 결제 영수증 / 만료 알림 |
...
5. Stripe Checkout works for all 3 tiers in test mode
6. `charge.refunded` triggers credit revocation
...
10. Refund policy + Terms + Privacy linked from Pricing footer
...
| TD-7 | GDPR / DSAR 자동화 부재 | 컴플라이언스 | Low (글로벌이지만 SMB) | v1.1 |
...
- `[TBD]` Stripe 결제 통화 — 전부 USD로 시작하되 EU VAT/Tax 자동 처리 옵션 활성화 여부
